...
namespace case_4
{
    public class RecommendationSystem
    {
        ...
        {
            var likedTracks = DatabaseHelper.GetUserTracks(UserId, "like_list");
            var dislikedTracks = DatabaseHelper.GetBlacklistedTracks(UserId);
            var ratedTracksInfo = DatabaseHelper.GetUserRatings(UserId);
            ...
            using (var connection = new SQLiteConnection(DatabaseHelper.ConnectionString))
            {
                ...
                {
                    var audio = DatabaseHelper.GetAudioById(ratedTrack.TrackId, connection);
                    if (audio != null)
                    {
        ...
        public List<Audio> GetAllTracksFromDatabase()
        {
            ...
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    tracks.Add(DatabaseHelper.CreateAudioFromReader(reader));
                }
            }
            return tracks;
        }
