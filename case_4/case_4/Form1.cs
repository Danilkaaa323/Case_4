...
namespace case_4
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string login)
        {
            ...
            CurrentUserId = DatabaseHelper.GetUserId(login);
            _recommendationSystem = new RecommendationSystem(CurrentUserId);
        ...
        {
            var userInfo = DatabaseHelper.GetUserInfo(CurrentUserLogin);
        ...
        {
            ListeningHistory = DatabaseHelper.GetUserTracks(CurrentUserId, "last_listened");
        ...
        {
            likelist_button.Text = $"Любимые треки ({DatabaseHelper.GetTrackListCount(CurrentUserId, "like_list")})";
            blacklist_button.Text = $"Не нравятся ({DatabaseHelper.GetBlacklistCount(CurrentUserId)})";
        }
        ...
        public void DisplayTrackInfo(Audio audio)
        {
            ...
            var info = new List<string>
            {
                ...
                "",
                $"BPM: {audio.Properties.BPM ?? 0}",
                $"Тональность: {audio.Properties.Key ?? "не указана"}",
                $"Акустичность: {FormatValue(audio.Properties.Acousticness)}",
                $"Энергия: {FormatValue(audio.Properties.Energy)}",
                $"Инструментальность: {FormatValue(audio.Properties.Instrumentalness)}",
                $"Речевая составляющая: {FormatValue(audio.Properties.Speechiness)}",
                $"Позитивность: {FormatValue(audio.Properties.Valence)}"
            };
        ...
        public string FormatValue(double? value) => value?.ToString("0.00") ?? "N/A";
        ...
        public Audio GetTrackById(int trackId)
        {
            using var connection = new SQLiteConnection(DatabaseHelper.ConnectionString);
            connection.Open();
            return DatabaseHelper.GetAudioById(trackId, connection);
        }
        ...
        {
            var likedTracks = DatabaseHelper.GetUserTracks(CurrentUserId, "like_list");
        ...
        private void Blacklist_button_Click(object sender, EventArgs e)
        {
            var blacklistedTracks = DatabaseHelper.GetBlacklistedTracks(CurrentUserId);
            LoadTracksIntoListBox(blacklistedTracks);
            ToggleListPanel(true);
            TracksTypelabel.Text = "Не нравятся";
        }
        ...
        private void Search_button_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                List<Audio> foundTracks = DatabaseHelper.SearchTracks(searchTerm);
        ...
        private async void Search_listBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (search_listBox.SelectedIndex >= 0 && search_listBox.SelectedIndex < SearchResults.Count)
            {
                ...
                await Task.Run(() => DatabaseHelper.AddToLastListened(CurrentUserId, CurrentTrack.ID));
                RefreshLastListenedTracks();
        ...
        private void Start_pictureBox_Click(object sender, EventArgs e) => StartStop(Start_pictureBox, Stop_pictureBox);
        private void Stop_pictureBox_Click(object sender, EventArgs e)
        {
            StartStop(Stop_pictureBox, Start_pictureBox);
        ...
        public void UpdateCurrentTrackRating()
        {
            if (CurrentTrack != null)
            {
                ...
                var rating = DatabaseHelper.GetTrackRating(CurrentUserId, CurrentTrack.ID);
            }
            else
            {
        ...
        {
            RatedTracks = DatabaseHelper.GetUserRatings(CurrentUserId);
        ...
        private void AcceptRaiting_label_Click(object sender, EventArgs e)
        {
            ...
            DatabaseHelper.AddOrUpdateRating(CurrentUserId, CurrentTrack.ID, rating);
            DatabaseHelper.AddToLastListened(CurrentUserId, CurrentTrack.ID);
        ...
        private void ListBoxTracks_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (CurrentTrack != null)
            {
                DatabaseHelper.AddToLastListened(CurrentUserId, trackId);
                RefreshLastListenedTracks();
                DisplayTrackInfo(CurrentTrack);
                UpdateCurrentTrackRating();
            }
        ...
        private void Start2_pictureBox_Click(object sender, EventArgs e)
        {
            ...
            StartStop(Start_pictureBox, Stop_pictureBox);
            Cursor.Current = Cursors.WaitCursor;
            Task.Run(() =>
            {
